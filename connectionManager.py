...
class ConnectionManager:
        ...
        try:
            ...
            register_micro_grid(MicroGridConfig(
                ...
                column_types={
                    ...
                    "trigger_event": pl.String,
                    "cron_expression": pl.String,
                    "filter_json": pl.String,
                    "message_template": pl.String,
                    "message_link": pl.String,
                    "message_styles": pl.String,
                    "priority": pl.String,
                    "due_warning_minutes": pl.String,
                    "allowed_states": pl.String,
                    "folder": pl.String,
                    "sound": pl.String,
                    "delivery": pl.String,
                    "room": pl.String,
            ...
            register_micro_grid(MicroGridConfig(
                ...
                column_types={
                    "index": pl.Int64,
                    "account": pl.String,
                    "clientTraderName": pl.String,
                    "clientTraderName2": pl.String,
                    "clientTraderName3": pl.String,
                    "salesName": pl.String,
                    "clientFlag": pl.String,
                    "asset": pl.String,
                    "altNames": pl.String,
                    "notes": pl.String,
                },
            ...
            register_micro_grid_group(MicroGridGroup(
                name="bot_config",
                display_name="Bot Configuration",
                grids=("bot_rules",),
            ))
            register_micro_grid_group(MicroGridGroup(
                name="clients",
            ...
            # --- Seed bot_rules with default rules if empty ---
            await self._seed_bot_rules()
    ...
    async def _seed_redist_params(self):
        ...
        try:
            ...
            _SEED_ROWS = [
                ...
                # -- Risk weights (must sum to 1.0, matches OptimizerConfig defaults) --
                {
                    "category"   : "risk_weight", "param_key": "NET_BSI",
                    "param_value": 0.40, "description": "Risk weight for net BSI exposure",
                    "min_value"  : 0.0, "max_value": 1.0
                },
                {
                    "category"   : "risk_weight", "param_key": "GROSS_BSI",
                    "param_value": 0.20, "description": "Risk weight for gross BSI exposure",
                    "min_value"  : 0.0, "max_value": 1.0
                },
                {
                    "category"   : "risk_weight", "param_key": "GROSS_BSR",
                    "param_value": 0.20, "description": "Risk weight for gross BSR exposure",
                    "min_value"  : 0.0, "max_value": 1.0
                },
                {
                    "category"   : "risk_weight", "param_key": "NET",
                    "param_value": 0.12, "description": "Risk weight for net position risk",
                    "min_value"  : 0.0, "max_value": 1.0
                },
                {
                    "category"   : "risk_weight", "param_key": "GROSS",
                    "param_value": 0.08, "description": "Risk weight for gross position risk",
                    "min_value"  : 0.0, "max_value": 1.0
                },
                # -- Solver defaults --
                {
                    "category"   : "solver_default", "param_key": "side_band_pct",
    ...
    async def _seed_bot_rules(self):
        """Populate bot_rules micro-grid with default message rules when empty."""
        from app.services.redux.micro_grid import get_micro_actor
        try:
            actor = get_micro_actor("bot_rules")
            if actor._data.hyper.height() > 0:
                return  # already seeded
            ...
            _SEED_ROWS = [
                {
                    "id": str(uuid.uuid4()), "name": "New Portfolio",
                    "enabled": "true", "icon": "\U0001f6a8",
                    "trigger_type": "event", "trigger_event": "new_portfolio",
                    "cron_expression": "", "filter_json": "{}",
                    "message_template": "{icon} {client} {side} {grossSize} | {assetClass} {venue}",
                    "message_link": "", "message_styles": '{"client":"sans_bold","side":"serif_bold"}',
                    "priority": "normal", "due_warning_minutes": "3",
                    "allowed_states": "", "folder": "", "sound": "", "delivery": "ib",
                    "room": "", "updated_at": "", "updated_by": "",
                },
                {
                    "id": str(uuid.uuid4()), "name": "Due in 3 Minutes",
                    "enabled": "true", "icon": "\u23f0",
                    "trigger_type": "event", "trigger_event": "due_warning",
                    "cron_expression": "", "filter_json": "{}",
                    "message_template": "{icon} Due in {due_warning_minutes}min: {client} {side} {grossSize}",
                    "message_link": "", "message_styles": '{"client":"sans_bold"}',
                    "priority": "high", "due_warning_minutes": "3",
                    "allowed_states": "LIVE,PENDING,NEW", "folder": "", "sound": "", "delivery": "ib",
                    "room": "", "updated_at": "", "updated_by": "",
                },
                {
                    "id": str(uuid.uuid4()), "name": "State Change",
                    "enabled": "true", "icon": "",
                    "trigger_type": "event", "trigger_event": "state_change",
                    "cron_expression": "", "filter_json": "{}",
                    "message_template": "{state_icon} {client} {side} {grossSize} \u2192 {new_state}",
                    "message_link": "", "message_styles": '{"new_state":"sans_bold"}',
                    "priority": "normal", "due_warning_minutes": "",
                    "allowed_states": "", "folder": "", "sound": "", "delivery": "ib",
                    "room": "", "updated_at": "", "updated_by": "",
                },
                {
                    "id": str(uuid.uuid4()), "name": "Warning",
                    "enabled": "true", "icon": "\u26a0\ufe0f",
                    "trigger_type": "event", "trigger_event": "warning",
                    "cron_expression": "", "filter_json": "{}",
                    "message_template": "{icon} Warning: {title} | {client} {side}",
                    "message_link": "", "message_styles": '{"title":"sans_bold"}',
                    "priority": "normal", "due_warning_minutes": "",
                    "allowed_states": "", "folder": "", "sound": "", "delivery": "ib",
                    "room": "", "updated_at": "", "updated_by": "",
                },
                {
                    "id": str(uuid.uuid4()), "name": "Error",
                    "enabled": "true", "icon": "\U0001F6AB",
                    "trigger_type": "event", "trigger_event": "error",
                    "cron_expression": "", "filter_json": "{}",
                    "message_template": "{icon} Error: {title} | {client}",
                    "message_link": "", "message_styles": '{"title":"sans_bold"}',
                    "priority": "high", "due_warning_minutes": "",
                    "allowed_states": "", "folder": "", "sound": "", "delivery": "ib",
                    "room": "", "updated_at": "", "updated_by": "",
                },
                {
                    "id": str(uuid.uuid4()), "name": "Success",
                    "enabled": "true", "icon": "\u2705",
                    "trigger_type": "event", "trigger_event": "success",
                    "cron_expression": "", "filter_json": "{}",
                    "message_template": "{icon} {title} | {client} {side}",
                    "message_link": "", "message_styles": '{}',
                    "priority": "normal", "due_warning_minutes": "",
                    "allowed_states": "", "folder": "", "sound": "", "delivery": "ib",
                    "room": "", "updated_at": "", "updated_by": "",
                },
            ...
            await actor.apply_edit({"add": seed_df}, user="system")
            await actor.persist()
            await log.info(f"[MicroGrid] Seeded bot_rules with {len(_SEED_ROWS)} default rules")

        except Exception as e:
            await log.error(f"[MicroGrid] Failed to seed bot_rules: {e}")
    ...
    async def _handle_unsubscribe(self, websocket: WebSocket, d: Dict[str, Any]):
        ...
        if not ok:
            return await self._send_direct_message(websocket, p.error(f"Failed to unsubscribe to: {p.context.room}"))
    ...
    async def _handle_upload(self, websocket: WebSocket, d: Dict[str, Any]):
        ...
        from app.config.config import APP_URL
        from app.services.loaders.load_sequence_v3 import load_portfolio

        trace = self._extract_trace(d)

        TOAST_PERSIST_OPTS = {"persist": True, "timeOut": 0, "extendedTimeOut": 0, "updateOnExist": True}
        ...
        try:
            ...
            start = time.time()
            my_pt, _ = await load_portfolio(pt_df.lazy(), dates=None, portfolio_key=portfolio_key, return_loader=False, broadcaster=self.ksm.ksm)
            load_time = time.time() - start

            # Send intermediate "Building meta..." toast
            await self._send_direct_message(
                websocket,
                Ack(
                    trace=trace,
            ...
            # Prepare client data for metadata
            client_data = {
                "client": client,
                "state": state,
                "rfqState": rfq_state,
            ...
            # Override date/time fields and compute derived datetime fields
            ...
            # Publish portfolio to KDB
            await self.ksm.ksm.publish_portfolio(portfolio_key, my_pt, my_meta)

            # Send success toast with clickable link
            portfolio_url = f'{APP_URL}/pt/{portfolio_key}'
            client_slug = client.replace(" ", "").upper()[:10]
            if client_slug:
                portfolio_url = f'{portfolio_url}?name={client_slug}'
            ...
            log.error(f"Failed to create portfolio {portfolio_key}: {str(e)}")
            return await self._send_direct_message(
                websocket,
                Ack(
                    trace=trace,
                    ...
                    toastType="error",
                    toastTitle="Upload",
                    toastMessage=f"Failed to create portfolio: {str(e)}",
                    toastId=toast_id,
                    toastOptions=TOAST_DONE_OPTS,
                ),
            )
    ...
    async def _handle_execute(self, websocket: WebSocket, d: Dict[str, Any]):
        ...
        try:
            import importlib
            mod = importlib.import_module("app.services.loaders.kdb_queries_dev_v3")
        ...
        try:
            ...
            if (skews is not None) and (not skews.hyper.is_empty()):
                ...
                my_skews = skews.lazy().with_columns(
                    [
                        pl.when(IS_PX).then(pl.col('refBid').cast(pl.Float64, strict=False)).otherwise(
                            pl.lit(None, pl.Float64)
                            ).alias('_refBidPx'),
                        pl.when(IS_PX).then(pl.col('refMid').cast(pl.Float64, strict=False)).otherwise(
                            pl.lit(None, pl.Float64)
                            ).alias('_refMidPx'),
                        pl.when(IS_PX).then(pl.col('refAsk').cast(pl.Float64, strict=False)).otherwise(
                            pl.lit(None, pl.Float64)
                            ).alias('_refAskPx'),
                        pl.when(IS_SPD).then(pl.col('refBid').cast(pl.Float64, strict=False)).otherwise(
                            pl.lit(None, pl.Float64)
                            ).alias('_refBidSpd'),
                        pl.when(IS_SPD).then(pl.col('refMid').cast(pl.Float64, strict=False)).otherwise(
                            pl.lit(None, pl.Float64)
                            ).alias('_refMidSpd'),
                        pl.when(IS_SPD).then(pl.col('refAsk').cast(pl.Float64, strict=False)).otherwise(
                            pl.lit(None, pl.Float64)
                            ).alias('_refAskSpd'),
                    ]
                ).join(
                    ...
                    [
                        pl.when(pl.col('_refBidPx').is_not_null()).then(pl.col('_refBidPx')).otherwise(
                            pl.col('_fallback_bid_px')
                            ).alias('_refBidPx'),
                        pl.when(pl.col('_refMidPx').is_not_null()).then(pl.col('_refMidPx')).otherwise(
                            pl.col('_fallback_mid_px')
                            ).alias('_refMidPx'),
                        pl.when(pl.col('_refAskPx').is_not_null()).then(pl.col('_refAskPx')).otherwise(
                            pl.col('_fallback_ask_px')
                            ).alias('_refAskPx'),
                        pl.when(pl.col('_refBidSpd').is_not_null()).then(pl.col('_refBidSpd')).otherwise(
                            pl.col('_fallback_bid_spd')
                            ).alias('_refBidSpd'),
                        pl.when(pl.col('_refMidSpd').is_not_null()).then(pl.col('_refMidSpd')).otherwise(
                            pl.col('_fallback_mid_spd')
                            ).alias('_refMidSpd'),
                        pl.when(pl.col('_refAskSpd').is_not_null()).then(pl.col('_refAskSpd')).otherwise(
                            pl.col('_fallback_ask_spd')
                            ).alias('_refAskSpd'),
                    ]
                ).select(
                    [
            ...
            grid_frame = grid_frame.with_columns(
                [
                    pl.col('avgLiqScore').hyper.fill_null(_DEFAULT_LIQ_SCORE, include_zero_as_null=True).alias(
                        'avgLiqScore'
                        ),
            ...
            def _opt_float(v, minimum=None, maximum=None):
                """Coerce to float, clamp to range, or None on failure."""
                if v is None:
                    return None
                try:
                    f = float(v)
            ...
            # These are internal calibration values that aren't exposed as sliders
            # (charge_strength curve, risk_weights blend, etc).
            charge_strength, default_strength, risk_weights, solver_defaults = \
                self._read_redist_params()

            # ─── Parse "Run From X" overrides ───
            # {"BUY|PX": 2.0, "SELL|SPD": -1.5} → {("BUY","PX"): 2.0, ...}
            def _parse_skew_override(raw):
                ...
                for k, v in raw.items():
                    ...
                    try:
                        ...
                        log.warning(f"[redistribute] Failed to parse skew override key '{k}': {e}")
                return result if result else None

            # ─── Build OptimizerConfig from the four user knobs + advanced options ───
            config = OptimizerConfig(
                # ── Core user knobs (from frontend sliders) ──
                target_blend=_opt_float(
                    options.get('target_blend'), minimum=0.0, maximum=1.0,
                ) if options.get('target_blend') is not None
                else solver_defaults.get('target_blend', 0.50),

                edge_strength=_opt_float(
                    options.get('edge_strength'), minimum=0.0, maximum=10.0,
                ) if options.get('edge_strength') is not None
                else solver_defaults.get('edge_strength', 1.0),

                liquidity_edge=_opt_float(
                    options.get('liquidity_edge'), minimum=-1.0, maximum=1.0,
                ) if options.get('liquidity_edge') is not None
                else solver_defaults.get('liquidity_edge', 1.0),

                trader_buffer_pct=_opt_float(
                    options.get('trader_buffer_pct'), minimum=0.05, maximum=1.0,
                ) if options.get('trader_buffer_pct') is not None
                else solver_defaults.get('trader_buffer_pct', 0.25),

                side_band_pct=_opt_float(
                    options.get('side_band_pct'), minimum=0.50, maximum=1.0,
                ) if options.get('side_band_pct') is not None
                else solver_defaults.get('side_band_pct', 0.99),

                # ── Advanced user knobs ──
                allow_through_mid=_opt_bool(
                    options.get('allow_through_mid'),
                    default=_opt_bool(solver_defaults.get('allow_through_mid', True), default=True),
                ),
                max_skew_delta_bps=_opt_float(options.get('max_skew_delta_bps'), minimum=0.0),
                max_skew_delta_pts=_opt_float(options.get('max_skew_delta_pts'), minimum=0.0),
                isolate_traders=_opt_bool(options.get('isolate_traders'), default=False),
            ...
            await log.debug(
                f"Entering solver: target_blend={config.target_blend:.3f}, "
                f"edge_strength={config.edge_strength:.3f}, "
                f"liquidity_edge={config.liquidity_edge:+.2f}, "
                f"trader_buffer_pct={config.trader_buffer_pct:.3f}, "
                f"side_band_pct={config.side_band_pct:.3f}",
                color="gray",
            )
            ...
            i_frame = await df_result.select(
                'tnum', 'final_skew', 'implied_px', 'implied_spd', 'quoteType'
                ).hyper.collect_async()
            by_skew = i_frame.join(j_frame, on='tnum', how='inner').partition_by(
                ['skewType', 'quoteType'], include_key=True, as_dict=True,
            )
            ...
            response_data = {
                ...
                "trader"       : (await summary_trader.sort(
                    [
                        pl.col('quoteType'),
                        pl.col('side'),
                        pl.col('wavg_skew_delta'),
                        pl.col('wavg_liq_score'),
                    ]
                ).hyper.collect_async()).to_dicts(),
                "updates"      : updates,
                "removed"      : removed_ids,
                "group_columns": group_columns,
            }
