...
class MicroGridActor:
    ...
    __slots__ = (
        ...
        "_persist_deleted_ids", "_changelog",
    )
        ...
        try:
            ...
            if df is not None and not df.hyper.is_empty():
                ...
                if keep:
                    ...
                    # Add missing schema columns with defaults (in-memory + DB)
                    missing_cols = [c for c in self.config.columns if c not in df.columns]
                    for col in missing_cols:
                        default = self.config.columns[col]
                        dtype = self.config.column_types.get(col, pl.String)
                        df = df.with_columns(pl.lit(default).cast(dtype).alias(col))
                    if missing_cols and self.config.persist:
                        # ALTER TABLE to add new columns so they persist on next save
                        for col in missing_cols:
                            dtype = self.config.column_types.get(col, pl.String)
                            sql_type = "TEXT" if dtype == pl.String else "REAL" if dtype in (pl.Float64, pl.Float32) else "INTEGER" if dtype in (pl.Int64, pl.Int32) else "TEXT"
                            try:
                                await db._backend_execute_write(
                                    f"ALTER TABLE `{table_name}` ADD COLUMN `{col}` {sql_type} DEFAULT ''",
                                    [], timeout_ms=5000,
                                )
                                await log.info(f"[MicroGrid] Added column `{col}` to {table_name}")
                            except Exception as e:
                                # "duplicate column" is expected if column was partially migrated
                                err_msg = str(e).lower()
                                if "duplicate" not in err_msg and "already exists" not in err_msg:
                                    await log.error(f"[MicroGrid] ALTER TABLE ADD `{col}` failed for {table_name}: {e}")
                        # Clear DB schema cache so upsert sees the new columns
                        await db._schema_cache.clear()
                    self._data = df
    ...
    # -- In-memory changelog (survives page refresh, clears on server restart) --

    def _log_change(self, type_: str, detail: str, user: str = "system") -> Dict[str, str]:
        ...
        if len(self._changelog) > self.CHANGELOG_MAX:
            self._changelog = self._changelog[-self.CHANGELOG_MAX:]
        return entry
    ...
    async def apply_edit(
        ...
        has_removes = False
        new_changelog_entries: List[Dict[str, str]] = []

        async with self._lock:
            current = self._data

            # Snapshot old rows as lookup for diffing (keyed by PK)
            _old_lookup: Dict[str, Dict[str, Any]] = {}
            try:
                for row in current.to_dicts():
                    _old_lookup[str(row.get(pk_col, ""))] = row
            except Exception:
                pass
            ...
            for frame in add_frames:
                ...
                # Changelog: log added rows
                for row_dict in frame.to_dicts():
                    summary = ", ".join(
                        f"{k}={v}" for k, v in row_dict.items()
                        if k != pk_col and v is not None and str(v) != ""
                    )[:120]
                    new_changelog_entries.append(
                        self._log_change("add", f"Row added: {summary or '(empty)'}", user)
                    )
            ...
            if update_frames:
                ...
                for frame in update_frames:
                    ...
                    for ucols, frames in grouped.items():
                        ...
                        # Changelog: log field-level diffs per updated row
                        for upd_row in batch.to_dicts():
                            row_pk = str(upd_row.get(pk_col, ""))
                            old_row = _old_lookup.get(row_pk)
                            if not old_row:
                                continue
                            diffs = []
                            for col in update_cols:
                                old_val = str(old_row.get(col, "") or "")
                                new_val = str(upd_row.get(col, "") or "")
                                if old_val != new_val:
                                    diffs.append(f"{col}: {old_val or '(empty)'} -> {new_val or '(empty)'}")
                            if diffs:
                                detail = ", ".join(diffs[:3])
                                new_changelog_entries.append(
                                    self._log_change("update", detail, user)
                                )
            ...
            for frame in remove_frames:
                ...
                if remove_mask.any():
                    has_removes = True
                    # Changelog: log removed rows before filtering
                    for rid in remove_id_set:
                        old_row = _old_lookup.get(rid)
                        if old_row:
                            summary = ", ".join(
                                f"{k}={v}" for k, v in old_row.items()
                                if k != pk_col and v is not None and str(v) != ""
                            )[:120]
                            new_changelog_entries.append(
                                self._log_change("remove", f"Row removed: {summary or '(empty)'}", user)
                            )
            ...
            if delta_parts:
                ...
                delta_df._micro_changelog = new_changelog_entries
            elif has_removes:
